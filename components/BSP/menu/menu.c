...
static void draw_wifi_line(int index, bool highlight)
{
    ...
    char buf[48];
    int y = 30 + index * 20; // 间距拉大，适应圆角矩形
    
    // 格式化输出：序号. SSID名称（限制13字符）
    snprintf(buf, sizeof(buf), "%2d. %-13.13s", index + 1, (char *)g_ap_records[index].ssid);

    // 擦除整行背景
    ili9341_draw_rectangle(0, y - 2, LCD_WIDTH, 20, LCD_COLOR_BLACK);

    if (highlight) {
        // 美化：选中项使用蓝色圆角矩形包裹
        ili9341_draw_round_rect(4, y - 2, LCD_WIDTH - 36, 18, 4, LCD_COLOR_BLUE);
        ili9341_draw_string_8x16(8, y, buf, LCD_COLOR_WHITE, LCD_COLOR_BLUE);
    } else {
        // 未选中项正常显示
        ili9341_draw_string_8x16(8, y, buf, LCD_COLOR_WHITE, LCD_COLOR_BLACK);
    }
...
static void display_detail(void)
{
    ...
    ili9341_fill_screen(LCD_COLOR_BLACK);
    ili9341_draw_rectangle(0, 0, LCD_WIDTH, 20, 0x3186); 
    ili9341_draw_string_8x16(5, 2, "AP Detail", LCD_COLOR_WHITE, 0x3186);
    ...
    ili9341_draw_string_8x16(10, LCD_HEIGHT - 32, "ENTER: Input Password", LCD_COLOR_GREEN, LCD_COLOR_BLACK);
    ili9341_draw_string_8x16(10, LCD_HEIGHT - 16, "BACK:  Return List", LCD_COLOR_GRAY, LCD_COLOR_BLACK);
}
...
static void display_keyboard(void)
{
    ...
    char title[64];
    // 显示连接的WiFi名称
    snprintf(title, sizeof(title), "Connect: %-12.12s", (char *)g_ap_records[sm.selected_index].ssid);
    ili9341_draw_string_8x16(5, 2, title, LCD_COLOR_CYAN, LCD_COLOR_BLACK);
    ...
    for (int r = 0; r < KB_ROWS; r++) {
        for (int c = 0; c < KB_COLS; c++) {
            if (kb_layout[r][c][0] == '\0') continue;  // 跳过空按钮

            int px = 4 + c * 23;  // X坐标计算
            int py = 46 + r * 18; // Y坐标计算
            uint16_t fg = LCD_COLOR_WHITE, bg = LCD_COLOR_BLACK;

            // 高亮当前光标所在位置
            if (r == sm.kb_cursor_row && c == sm.kb_cursor_col) {
                fg = LCD_COLOR_BLACK; bg = LCD_COLOR_WHITE; // 高亮
            }
            ili9341_draw_rectangle(px, py, 21, 16, bg);
            int text_x = px + (21 - strlen(kb_layout[r][c]) * 8) / 2;  // 居中对齐
...
static void enter_state(menu_state_t new_state)
{
    ...
    switch (new_state) {
        case STATE_LIST:        display_wifi_list(); break;
        case STATE_DETAIL:      display_detail();    break;
        case STATE_KEYBOARD:
            // 初始化键盘光标位置
            sm.kb_cursor_row = 0; sm.kb_cursor_col = 0;
            sm.kb_last_row = -1; sm.kb_last_col = -1;
            display_keyboard();
            break;
        ...
        case STATE_CONNECTING:
            {
                ili9341_fill_screen(LCD_COLOR_BLACK);
                ili9341_draw_round_rect(20, 40, LCD_WIDTH - 40, 80, 5, 0x2104); // 暗底框
                ili9341_draw_string_8x16(30, 50, "Connecting...", LCD_COLOR_YELLOW, 0x2104);
                
                // 美化：加入进度条UI
                ili9341_draw_progress_bar(30, 95, LCD_WIDTH - 60, 10, 50, LCD_COLOR_GREEN, LCD_COLOR_BLACK);
        ...
        case STATE_SMARTCONFIG:
            {
                ...
                ili9341_draw_string_8x16(10, 30, "Waiting for ESP Touch...", LCD_COLOR_WHITE, LCD_COLOR_BLACK);
                ili9341_draw_progress_bar(10, 60, LCD_WIDTH - 20, 10, 50, LCD_COLOR_CYAN, LCD_COLOR_BLACK);
...
{
    if (g_ap_count == 0 && event != BUTTON_BACK && event != BUTTON_RIGHT) return;
    ...
    switch (event) {
        case BUTTON_UP:
            ...
            draw_wifi_line(old, false); draw_wifi_line(sm.selected_index, true);
            break;
        case BUTTON_DOWN:
            // 循环选择，向下滚动
...
static void handle_keyboard_event(button_event_t event)
{
    ...
    switch (event) {
        case BUTTON_UP:
            // 在键盘上向上移动，跳过空按钮
            do { sm.kb_cursor_row = (sm.kb_cursor_row - 1 + KB_ROWS) % KB_ROWS; } 
            while (kb_layout[sm.kb_cursor_row][sm.kb_cursor_col][0] == '\0');
            update_keyboard_cursor();
            break;
        case BUTTON_DOWN:
            // 在键盘上向下移动，跳过空按钮
            do { sm.kb_cursor_row = (sm.kb_cursor_row + 1) % KB_ROWS; } 
            while (kb_layout[sm.kb_cursor_row][sm.kb_cursor_col][0] == '\0');
            update_keyboard_cursor();
            break;
        case BUTTON_RIGHT:
            // 在键盘上向右移动，跳过空按钮
            do { sm.kb_cursor_col = (sm.kb_cursor_col + 1) % KB_COLS; } 
            while (kb_layout[sm.kb_cursor_row][sm.kb_cursor_col][0] == '\0');
            update_keyboard_cursor();
            break;
        case BUTTON_BACK:
        ...
        case BUTTON_ENTER:
            {
                ...
                if (strcmp(key, "CLR") == 0) {
                    // 清空密码
                    sm.password_len = 0; sm.password[0] = '\0';
                } else if (strcmp(key, "SPC") == 0) {
                    // 添加空格
                    if (sm.password_len < 63) sm.password[sm.password_len++] = ' ';
...
static void dispatch_button_event(button_event_t event)
{
    ...
    switch (sm.state) {
        case STATE_LIST:        
            handle_list_event(event); break;
        case STATE_DETAIL:      
            if (event == BUTTON_BACK) enter_state(STATE_LIST);
            if (event == BUTTON_ENTER) enter_state(STATE_KEYBOARD);
            break;
        case STATE_KEYBOARD:    
            handle_keyboard_event(event); break;
        case STATE_CONNECTING:  
        case STATE_SMARTCONFIG: 
            // 在连接或SmartConfig状态下，按回车键释放信号量
            if (event == BUTTON_ENTER) xSemaphoreGive(s_connect_done_sem);
            break;
    }
}
...
static void IRAM_ATTR gpio_isr_handler(void *arg)
{
    ...
    else if (gpio_num == KEY_RIGHT_GPIO) event = BUTTON_RIGHT;
    else if (gpio_num == KEY_ENTER_GPIO) event = BUTTON_ENTER;
    else if (gpio_num == KEY_BACK_GPIO)  event = BUTTON_BACK;
    else return;
    ...
    while (1) {
        if (xQueueReceive(s_button_queue, &event, portMAX_DELAY) == pdTRUE) {
            ...
            switch (event) {
                ...
                case BUTTON_RIGHT: pressed = (gpio_get_level(KEY_RIGHT_GPIO) == 0); break;
                case BUTTON_ENTER: pressed = (gpio_get_level(KEY_ENTER_GPIO) == 0); break;
                case BUTTON_BACK:  pressed = (gpio_get_level(KEY_BACK_GPIO) == 0); break;
                default: break;
            }
            if (!pressed) continue;
